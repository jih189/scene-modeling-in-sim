...
def inference(global_config, checkpoint_dir, input_paths, K=None, local_regions=True, skip_border_objects=False, filter_grasps=True, segmap_id=None, z_range=[0.2,1.8], forward_passes=1):
    ...
    for p in glob.glob(input_paths):
        ...
        print("x max", np.max(pc_full[:,0]))
        print("x min", np.min(pc_full[:,0]))
        print("y max", np.max(pc_full[:,1]))
        print("y min", np.min(pc_full[:,1]))
        print("z max", np.max(pc_full[:,2]))
        print("z min", np.min(pc_full[:,2]))
        print("pc_segments", pc_segments[1.0].shape)
        print("size of segmentations mask", len(pc_segments))
